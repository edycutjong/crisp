...
test.describe("Crisp Solvency & Attestation Flows", () => {
  ...
  test("should verify customer balance inclusion successfully", async ({ page }) => {
    await page.goto("/");
    ...
    await publicKeyInput.fill("GA111111111111111111111111111111111111111111111111111111");
    ...
    await expect(page.locator("text=Inclusion Verified!")).toBeVisible({ timeout: 10000 });
  ...
  test("should publish a new solvency attestation successfully", async ({ page }) => {
    await page.goto("/");

    // 1. Connect Issuer Wallet
    const connectBtn = page.getByRole("button", { name: "Connect Freighter Wallet" });
    await expect(connectBtn).toBeVisible();
    await connectBtn.click();

    // Verify wallet connected state
    await expect(page.locator("text=Address: GDISSUER")).toBeVisible({ timeout: 5000 });
    ...
    const publishBtn = page.getByRole("button", { name: "Publish Solvency Proof" });
    await publishBtn.click();

    // Check step-by-step logs streaming
    await expect(page.locator("text=Constructing Merkle-Sum Tree")).toBeVisible({ timeout: 5000 });
    await expect(page.locator("text=Generating ZK Proof")).toBeVisible({ timeout: 5000 });
    await expect(page.locator("text=Submitting attestation to Soroban")).toBeVisible({ timeout: 5000 });

    // Wait for final success message
    await expect(page.locator("text=Solvency Attestation Successfully Published!")).toBeVisible({ timeout: 15000 });
  });

  test("should reject attestation when reserves are less than liabilities", async ({ page }) => {
    await page.goto("/");
    ...
    await expect(page.locator("text=invariant violated").first()).toBeVisible({ timeout: 15000 });
  });
});
