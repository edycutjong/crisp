# Changelog

All notable changes to this project will be documented in this file.

## [1.8.0](../../compare/v1.7.0...v1.8.0) (2026-07-02)

### 🚀 Features

- **zk:** witness the real on-chain proof in-browser (7154965)

## [1.7.0](../../compare/v1.6.0...v1.7.0) (2026-07-02)

### 🚀 Features

- **ui:** show app version badge in footer (0b4ca80)

## [1.6.0](../../compare/v1.5.0...v1.6.0) (2026-07-02)

### 🚀 Features

- **seo:** robots + sitemap, security headers (9835fdc)

## [1.5.0](../../compare/v1.4.0...v1.5.0) (2026-07-01)

### 🚀 Features

- **legal:** add Privacy & Terms pages + footer links (6a6681c)

## [1.4.0](../../compare/v1.3.0...v1.4.0) (2026-07-01)

### 🚀 Features

- **ui:** issuer Disconnect button and custom 404 page (075043b)

## [1.3.0](../../compare/v1.2.0...v1.3.0) (2026-07-01)

### 🚀 Features

- **wallet:** fix Freighter detection gate + add demo button (55a00a2)

### 💄 Style

- fix prettier formatting (7f6234a)
- format pitch.html with prettier (41f833e)

### ✅ Tests

- **core:** verify and complete unit test suites and coverage (5e074a2)

### 📝 Documentation

- **pitch:** replace page 1 emojis with SVG icons (71a3c2a)

## [1.2.0](../../compare/v1.1.1...v1.2.0) (2026-06-29)

### 🚀 Features

- **crisp:** update contract ids to real testnet deployments (82adf98)

### 🐛 Bug Fixes

- **e2e:** fix solvency attestation race conditions in E2E tests (e1ae51e)
- **crisp:** resolve eslint errors and unit test failure (2871f92)

### 📝 Documentation

- **pitch:** replace emoji with logo icon and update demo video link (a0416bc)
- **readme:** add walkthrough screenshot gallery to README (5410fe2)
- **readme:** update demo video YouTube URL and configuration (a868360)
- **readme:** update readme and correct repository link (4314c0a)
- **readme:** add Demo Materials section and link GitHub & Pitch Deck (0f88409)

### 💄 Style

- **crisp:** run prettier to format codebase (2d596ba)

## [1.1.1](../../compare/v1.1.0...v1.1.1) (2026-06-28)

### 🐛 Bug Fixes

- **components:** replace inline SVG logo with public icon.svg in navbar (2e074d9)

## [1.1.0](../../compare/HEAD~50...v1.1.0) (2026-06-28)

### 🚀 Features

- **setup:** initialize project codebase (01edeac)

### 🐛 Bug Fixes

- **ci:** increase bundle size limits and apply code formatting (aa52d85)
- **package:** update homepage url to custom domain crisp.edycu.dev (b3eefe4)
- **ci:** add package repo metadata, sync lockfile, and upgrade CI to Node 22 (dc9af57)

### 🤖 CI/CD

- **deploy:** add Vercel CLI deployment steps to CI workflow (85d200e)

