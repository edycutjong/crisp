...
const defaultUsers = [
  { accountId: 'GA111111111111111111111111111111111111111111111111111111', balance: 100000n, salt: 'a3c1' },
  { accountId: 'GA222222222222222222222222222222222222222222222222222222', balance: 150000n, salt: 'b8e2' },
  { accountId: 'GA333333333333333333333333333333333333333333333333333333', balance: 50000n, salt: 'c9d3' },
  { accountId: 'GA444444444444444444444444444444444444444444444444444444', balance: 200000n, salt: 'd1f4' }
];

async function fetchHorizonBalances() {
  try {
    const res = await fetch("https://horizon-testnet.stellar.org/accounts?asset=USDC:GA2FZDW4SABBXZ7XNG6KBNSH62NMXEW464RDAZEW6GQDBBENAX2LC43A&limit=10");
    if (res.ok) {
    ...
    console.warn("Horizon balance scrape in seeder failed, using defaults:", err.message);
  }
  return defaultUsers;
}
...
async function seed() {
  ...
  console.log(`Scraped ${users.length} active USDC accounts from Horizon Testnet.`);
  ...
  // Issuer details
  const issuerAddress = 'GDISSUERXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX';
  const txHash = '8888888888888888888888888888888888888888888888888888888888888888';
  
  console.log('Inserting Solvency Report into Supabase...');
  
  // 1. Delete old rows to prevent key conflicts
  await supabase.from('crisp_user_balance_proofs').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  await supabase.from('crisp_solvency_reports').delete().neq('id', '00000000-0000-0000-0000-000000000000');
