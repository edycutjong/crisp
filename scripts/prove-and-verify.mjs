...
const ORACLE = process.env.ORACLE_ID || "CDXROOACFGK7FIOMNRO22O25O5YIMSHA3DKEIQXUUWHR74QGVGKXXSOY";
...
async function run() {
  ...
  execFileSync("node", [`${C}/gen_solvency_js/generate_witness.js`, `${C}/gen_solvency_js/gen_solvency.wasm`,
    "/dev/stdin", `${C}/_g.wtns`], { input: JSON.stringify({ accountIds: ids, balances, salts }) });
  execSync(`npx snarkjs wtns export json ${C}/_g.wtns ${C}/_g.json`, { stdio: "ignore" });
  ...
  const input = {
    expectedLiabilitiesRoot: root, expectedLiabilitiesSum: sum.toString(), reserves: reserves.toString(),
    issuerAx, issuerAy,
    accountIds: ids, balances, salts,
    sigS: sig.S.toString(),
    sigR8x: F.toObject(sig.R8[0]).toString(),
    sigR8y: F.toObject(sig.R8[1]).toString(),
  };
  console.log(`Generating real BN254 Groth16 solvency proof (EdDSA-signed; liabilities ${sum}, reserves ${reserves})...`);
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(
    input, `${C}/solvency_js/solvency.wasm`, `${C}/s_final.zkey`);
  ...
  const out = execFileSync("stellar", [
    "contract", "invoke", "--id", ORACLE, "--source", "deployer", "--network", "testnet", "--send=yes",
    "--", "attest_reserves", "--proof", proofHex, "--kyc_root", rootHex,
    "--total_liabilities", sum.toString(), "--reserves_threshold", reserves.toString(),
    "--issuer_ax", issuerAxHex, "--issuer_ay", issuerAyHex,
...
run().then(() => process.exit(0)).catch((e) => { console.error(String(e).split("\n")[0]); process.exit(1); });
