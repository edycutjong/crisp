  ...
  exports: {
    createClient: () => ({
      from: (table) => ({
        select: () => ({
          order: () => ({
            limit: () => Promise.resolve(global.supabaseMockConfig.selectReportResult)
          }),
          eq: () => ({
            limit: () => Promise.resolve(global.supabaseMockConfig.selectProofResult)
          })
        }),
        delete: () => ({
        ...
        insert: () => {
          if (table === "crisp_solvency_reports") {
            return Promise.resolve(global.supabaseMockConfig.insertReportResult);
          } else {
            return Promise.resolve(global.supabaseMockConfig.insertProofsResult);
          }
        }
      })
    })
...
const { MerkleSumTree, hashPoseidon2, hashSha256 } = require('../src/lib/merkleSumTree');
...
test('Hash functions should return correct formats', async () => {
  ...
  assert(sha !== pos, 'Different hash functions should yield different results');
});
...
for (let numLeaves = 1; numLeaves <= 16; numLeaves++) {
  test(`Tree sizing, padding and aggregation checks for N = ${numLeaves} leaves`, async () => {
    ...
    let expectedSum = 0n;
    mockLeaves.forEach(l => { expectedSum += l.balance; });
    assert(root.sum === expectedSum, `Root sum should match expected summation: ${root.sum} !== ${expectedSum}`);
    ...
    assert(tree.layers[0].length === expectedPaddedLength, `Tree layer 0 should be padded to ${expectedPaddedLength}`);
  });
  ...
  test(`Inclusion proof path verification for N = ${numLeaves} leaves`, async () => {
    ...
    for (const leaf of mockLeaves) {
      ...
      assert(isVerified, `Proof should verify successfully for account: ${leaf.accountId}`);
    }
  });
}
...
test('Verification should reject modified balance inputs', async () => {
  const leaves = [
    { accountId: 'GA111111111111111111111111111111111111111111111111111111', balance: 10000n, salt: 's1' },
    { accountId: 'GA222222222222222222222222222222222222222222222222222222', balance: 20000n, salt: 's2' }
  ];
...
test('Verification should reject modified sibling path values', async () => {
  ...
  assert(!isVerified, 'Verification should fail on altered sibling sum in path');
});
...
test('Anomalous dataset: Check negative balance leaf detection', async () => {
  ...
  try {
    ...
    assert(tree.getRoot().sum < 0n, 'Sum of negative balances should result in negative root liabilities');
    
    // Ensure we flag this on circuit validation level
    console.log('   - Negative balance attack correctly verified: root sum computes to negative value');
  } catch (err) {
...
test('Anomalous dataset: Duplicate account ID collision check', async () => {
  const leaves = [
    { accountId: 'GACOLLISION11111111111111111111111111111111111111111111', balance: 10000n, salt: 's1' },
    { accountId: 'GACOLLISION11111111111111111111111111111111111111111111', balance: 20000n, salt: 's2' } // same ID
  ];
  ...
  assert(leaf1.hash !== leaf2.hash, 'Salts should prevent collision for identical Account IDs');
});

test('Tree check: getRoot throws if tree not built', () => {
  const tree = new MerkleSumTree([{ accountId: 'GA1', balance: 1000n, salt: 's1' }]);
  try {
    tree.getRoot();
    assert(false, 'Should have thrown on unbuilt tree getRoot');
  } catch (err) {
    assert(err.message.includes('not built'), 'Error should mention tree not built');
  }
});
...
test('Tree check: getProof throws if account not found', async () => {
  ...
  try {
    ...
    assert(err.message.includes('not found'), 'Error should mention account not found');
  }
});
...
test('DB: In-memory mock mode and real Supabase client path validation', async () => {
  ...
  const { getLatestReport, getProofForAccount, insertNewAttestation, getSupabase } = require('../src/lib/db');
  ...
  assert(immediateReport === null, "Should return null before seeding completes");
  ...
  assert(report.total_reserves === 520000, "Mock report total_reserves should match seeded value");
  
  const proof = await getProofForAccount("GA111111111111111111111111111111111111111111111111111111");
  ...
  await insertNewAttestation(
    { issuer_address: "issuer", tx_hash: "tx", total_liabilities: 100, total_reserves: 200, kyc_root: "non-matching-root", timestamp: "now" },
    [{ kyc_root: "another-root", account_address: "GA_NON_MATCH", balance: 100, proof_path: [] }]
  );
  const nonMatchProof = await getProofForAccount("GA_NON_MATCH");
  assert(nonMatchProof.solvency_reports === null, "Should fallback to null for non-matching report");
  ...
  assert(reportReal.total_liabilities === 100, "Should return mocked select data");

  const proofReal = await getProofForAccount("GA_TEST");
  assert(proofReal.balance === 100, "Should return mocked select data");

  await insertNewAttestation(
    { issuer_address: "issuer", tx_hash: "tx", total_liabilities: 100, total_reserves: 200, kyc_root: "root", timestamp: "now" },
    [{ kyc_root: "root", account_address: "acc", balance: 100, proof_path: [] }]
  );
...
test('DB: Seeding error fallback validation', async () => {
  ...
  console.error = (...args) => {
    if (args[0] && typeof args[0] === 'string' && args[0].includes("seeding mock db")) {
      errorLogged = true;
    } else {
  ...
  assert(errorLogged, "Error block inside seedInMemory catch should be executed");
});

test('ZK: serializeProof, rootHex, and CrispProver proveSolvency checks', async () => {
  const { serializeProof, rootHex, CrispProver } = require('../src/lib/zkProver');
  ...
  assert(res.totalLiabilities === "1000", "Total liabilities should be correct");
...
test('ZK: CrispProver dynamic import test (without mock)', async () => {
  ...
  assert(res2 && res2.groth16 !== undefined, "Should return imported snarkjs when global is deleted");

  global.snarkjs = savedMock;
});

test('DB: Supabase error and fallback paths statement coverage', async () => {
  const { getLatestReport, getProofForAccount, insertNewAttestation, getIsMock, getSupabase } = require('../src/lib/db');
  ...
  global.supabaseMockConfig.selectReportResult = { data: null, error: new Error("Simulated query error for report") };
  try {
  ...
  global.supabaseMockConfig.selectProofResult = { data: null, error: new Error("Simulated query error for proof") };
  try {
  ...
  global.supabaseMockConfig.deleteResult = { error: new Error("Simulated delete error") };
  ...
  global.supabaseMockConfig.insertReportResult = { error: new Error("Simulated report insert error") };
  ...
  global.supabaseMockConfig.insertProofsResult = { error: new Error("Simulated proofs insert error") };
  try {
    await insertNewAttestation(
      { issuer_address: "issuer", tx_hash: "tx", total_liabilities: 100, total_reserves: 200, kyc_root: "root", timestamp: "now" },
      [{ kyc_root: "root", account_address: "acc", balance: 100, proof_path: [] }]
  ...
  global.supabaseMockConfig.selectReportResult = { data: [{ total_liabilities: 100 }], error: null };
  global.supabaseMockConfig.selectProofResult = { data: [{ balance: 100 }], error: null };
...
test('MerkleSumTree: SHA-256 branch coverage', async () => {
  ...
  assert(tree.getRoot().sum === 100n, "Root sum should be correct with SHA-256");
  assert(tree.getRoot().hash.length > 0, "Root hash should be generated with SHA-256");
  ...
  assert(verifiedDefault === true, "Verification should pass with default Poseidon");
});
