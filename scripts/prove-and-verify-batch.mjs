...
const ORACLE = process.env.BATCH_ORACLE_ID || "CANW4N5YTB4UYDM4MO5WK5SUHGPLJBXG3FQATLZQ5QKAQ2A57TXQ2DL2";
...
async function run() {
  ...
  const input = { total_system_liabilities, total_system_reserves, issuer_roots, issuer_liabilities, issuer_reserves };

  console.log(`Generating real BN254 Groth16 batch proof (4 issuers; sys liabilities ${total_system_liabilities}, reserves ${total_system_reserves})...`);
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(
    input, `${C}/aggregator_js/aggregator.wasm`, `${C}/agg_final.zkey`);
  const vk = (await import(`${C}/agg_vk.json`, { with: { type: "json" } })).default;
  ...
  const invoke = (args) => execFileSync("stellar", [
    "contract", "invoke", "--id", ORACLE, "--source", "deployer", "--network", "testnet", ...args,
  ...
  const result = invoke(["--", "verify_batch_proof", "--proof", proofHex, "--batch_root", batchRootHex,
    "--total_system_liabilities", liab, "--total_system_reserves", res]);
  ...
  const tampered = invoke(["--", "verify_batch_proof", "--proof", proofHex, "--batch_root", batchRootHex,
    "--total_system_liabilities", String(BigInt(liab) + 1n), "--total_system_reserves", res]);
